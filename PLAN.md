...
PowerShell 7.4+ modules that ship .NET assemblies (e.g. `MicrosoftTeams`, `Az.*`, `Microsoft.Graph.*`)
are notorious for "assembly version hell": multiple modules bundle different, sometimes incompatible,
versions of the same DLL (e.g. `Microsoft.Identity.Client`, `Newtonsoft.Json`,
`Microsoft.ApplicationInsights`). Because PowerShell 7.4+ loads everything into a single default
`AssemblyLoadContext` (the default `AssemblyLoadContext.Default` in PS 7 / .NET Core/.NET), the first
module to load a given assembly "wins", and every other module that expects a different version can
fail with `FileLoadException`, `MethodMissingException`, or silent incorrect behavior.

`PowerShellRepackager` is a tool that repackages an existing PowerShell Gallery module (targeting
PowerShell 7.4+) so that its bundled .NET assemblies are isolated in a private, named
`AssemblyLoadContext` (ALC). This removes cross-module DLL collisions without needing to fork/recompile
the module's source.
...
**Scope:** This tool is designed exclusively for **PowerShell 7.4 and later** targeting .NET Core and
modern .NET (netcore3.1, net5.0, net6.0, net7.0, net8.0+). Windows PowerShell 5.1 / .NET Framework 
modules are explicitly out of scope and will be skipped during repackaging, resulting in smaller final 
packages.
...
- Identify and separate:
  ...
  - The root module script/binary (`RootModule` entry in the manifest — could be a `.psm1`, `.dll`, or
    absent).
  - **All bundled binary assemblies for PowerShell 7.4+ only:** Recursively scan for `*.dll` files
    **only** in .NET Core and modern .NET target framework folders (`bin/netcore3.1`, `bin/net5.0`, 
    `bin/net6.0`, `bin/net7.0`, `bin/net8.0`, `bin/net9.0`, etc.). Explicitly **skip and discard** any 
    .NET Framework / Windows PowerShell 5.1 related folders (`bin/net472`, `bin/net48`, `bin/desktop`, 
    `bin/core`), as they are not compatible with PowerShell 7.4+ and would only bloat the repackaged 
    module.
...
- **Output:** a normalized in-memory (or on-disk) model of:
  ...
  - **Detected .NET Core / modern .NET target framework folder** for assemblies (e.g., `netcore3.1`, 
    `net8.0`) — the ALC loader will be configured for this single TFM, simplifying the pipeline compared 
    to supporting multiple runtime variants.
  - Note: if the package contains only .NET Framework / Windows PowerShell assemblies and no .NET Core 
    or modern .NET assemblies, fail with a clear message that the module is not compatible with 
    PowerShell 7.4+ and cannot be repackaged by this tool.

### Step 3 — Generate the Assembly Load Context loader

Goal: ensure that when the module is imported on PowerShell 7.4+, all of its bundled DLLs are loaded
from a dedicated, named `AssemblyLoadContext` instead of `AssemblyLoadContext.Default`, so that
identically-named assemblies with different versions used by other modules never collide.
...
- **Design:** generate a small C# "loader" project per repackaged module that:
  1. Defines a custom `AssemblyLoadContext` subclass, e.g. `Svrooij{Name}LoadContext`, constructed with
     ...
     (resolved relative to the assembly's own location, not a hardcoded path, and compatible with 
     any of: `netcore3.1`, `net5.0`, `net6.0`, `net7.0`, `net8.0`, etc.).
  ...
  4. Is careful about **type identity**: any exported type that PowerShell itself binds to (e.g.
     ...
     libraries, telemetry SDKs, etc.) should be isolated. The loader design includes a small "shared /
...
- **Output:** loader source is *generated* (templated) per module, compiled into a small
  `Svrooij.{Name}.Loader.dll`, and this loader DLL is the one placed into the module's entry point.
- **Build:** the generator will:
  - Ship a single prebuilt generic "loader" library once, parameterized at runtime via a config file /
    embedded resource listing the module's bin folder and the list of DLL names to isolate (**preferred
    — avoids per-module compilation, faster, no build toolchain dependency at repackage time**).
  - "Generating the loader" in Step 3 becomes "generate a small config artifact + copy the prebuilt
    generic loader DLL into the package", not "generate and compile new C# per module".
  - This avoids needing a full C# compiler at repackage time and is much more robust.

**PowerShell 7.4+ Simplification:** Since we target only modern PowerShell, we use a single
`AssemblyLoadContext` per module, avoiding the complexity of managing multiple runtime variants
(classic vs. core). This streamlines both the loader implementation and the repackaging process.
...
- **PowerShell 7.4+ only:** The repackager explicitly targets PowerShell 7.4 and later. Modules compiled
  for Windows PowerShell 5.1 or .NET Framework will be detected and skipped. This significantly reduces
  package size and complexity, as there is no need to maintain dual-runtime support or classic AppDomain
  compatibility.
  - During Step 2 (extraction), any .NET Framework assemblies (`net472`, `net48`, `desktop`, `core`)
    are excluded from processing and not carried forward to the repackaged module.
  - .NET Core and modern .NET folders (`netcore3.1`, `net5.0`, `net6.0`, `net7.0`, `net8.0`, etc.) are 
    recognized and preserved.
  - If a module contains only .NET Framework assemblies, the repackager will fail with a clear message
    indicating the module is not compatible with PowerShell 7.4+ and cannot be repackaged.
...
- **Simplified TFM handling:** Because we only support PowerShell 7.4+, we target a single .NET Core or 
  modern .NET TFM (e.g., `netcore3.1`, `net8.0`). The ALC loader is configured for this single framework, 
  eliminating the complexity of managing multiple runtime-specific DLL sets. If a package includes multiple 
  .NET Core or modern .NET folders (e.g., both `netcore3.1` and `net8.0`), the highest compatible version 
  is selected and others are pruned.

- **Safety / correctness validation:** after repackaging, run an automated smoke test — import the new
  module in a clean `pwsh -NoProfile` process (PowerShell 7.4+), list exported commands, and (if
  feasible) invoke at least one non-networked cmdlet (e.g. `Get-Help`, a `-WhatIf` no-op) to catch
  obvious loader breakage before declaring success.
...
1. Exact TFM selection strategy: if a module ships multiple .NET Core or modern .NET folders 
   (e.g., netcore3.1, net6.0, net7.0, net8.0), should we select the highest version compatible with the 
   host PowerShell 7.4+ environment, or allow the user to override which TFM to use?
...
5. Target implementation language/host: a PowerShell module (7.4+) calling into a small companion .NET
